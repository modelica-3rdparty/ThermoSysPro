...
model StaticExchangerKS_new_abs
...
equation
  ...
  if (exchanger_conf == 1) then
     ...
     //DT1 = max(Tec - Tsf, 1e-6);
     //DT2 = max(Tsc - Tef, 1e-6);
     DT1 = Tec - Tsf;
     DT2 = Tsc - Tef;
  ...
  DTln=(DT1-DT2)/Modelica.Math.log(abs(DT1/DT2)); //should not be used
  //DTln=(DT1-DT2)/(Modelica.Math.log(DT1) - Modelica.Math.log(DT2));
  QcCpc = Qc*Cpc;
  QfCpf = Qf*Cpf;
