...
model StaticExchangerKS_new_DT1DT2
...
equation
  ...
  DT2OldMethod = if (exchanger_conf == 1) then DT1*Modelica.Math.exp(-Kcor*K*S*(1/(Qc*Cpc) - 1/(Qf*Cpf)))
  ...
  QcCpc = Qc*Cpc;
  QfCpf = Qf*Cpf;
  DifQcCpcQfCpf = 1/(Qc*Cpc) - 1/(Qf*Cpf);
...
end StaticExchangerKS_new_DT1DT2;
