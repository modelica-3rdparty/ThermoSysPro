...
algorithm
  if p < 8.7075e5 then
    ...
    if p < 93.285e5 then
      h := ThermoSysPro.Properties.WaterSteamSimple.Utilities.polynomial_x_order5(coef2, p);
    else
      h := ThermoSysPro.Properties.WaterSteamSimple.Utilities.polynomial_x_order7(coef3, p);
    end if;
  end if;
