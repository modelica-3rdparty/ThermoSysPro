...
algorithm
  ...
  elseif rho < d2sat then
    ...
    lambda := ThermoSysPro.Properties.WaterSteamSimple.Conductivity.lambda1_dT(d1sat, T)*(1 - x) + ThermoSysPro.Properties.WaterSteamSimple.Conductivity.lambda2_dT(d2sat, T)*x;
  end if;
  annotation(
    smoothOrder = 2,
