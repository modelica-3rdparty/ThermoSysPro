...
function derThermalConductivity_derT_derrho "der(Thermal Conductivity) computation for Dry Air Ideal Gas (inputs: T, rho, der(T), der(rho))"
...
// der_k = derivative of diluteConductivity*lambda with respect to T times der_T + derivative of residualConductivity*lambda with respect to rho times der_rho
  der_k := (lambda*(C1/tnorm + C05/(2*sqrt(T/tnorm)*tnorm) - (C_1*tnorm)/T^2 - (2*C_2*tnorm^2)/T^3 - (3*C_3*tnorm^3)/T^4 - (4*C_4*tnorm^4)/T^5))*der_T + (lambda*((5*D5*rho^4)/rhonorm^5 + (4*D4*rho^3)/rhonorm^4 + (3*D3*rho^2)/rhonorm^3 + (2*D2*rho)/rhonorm^2 + D1/rhonorm))*der_rho;
  annotation(
