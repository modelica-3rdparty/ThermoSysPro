...
function derDynamicViscosity_derT_derrho "der(Dynamic Viscosity) computation for Dry Air Ideal Gas (inputs: T, rho, der(T), der(rho))"
...
// der_mu = derivative of viscosityZeroDensity*H with respect to T times der_T + derivative of residualViscosity*H with respect to rho times der_rho
  der_mu := (H*(A1/tnorm + A05/(2*sqrt(T/tnorm)*tnorm) - (A_1*tnorm)/T^2 - (2*A_2*tnorm^2)/T^3 - (3*A_3*tnorm^3)/T^4 - (4*A_4*tnorm^4)/T^5))*der_T + (H*((4*B4*rho^3)/rhonorm^4 + (3*B3*rho^2)/rhonorm^3 + (2*B2*rho)/rhonorm^2 + B1/rhonorm))*der_rho;
  annotation(
