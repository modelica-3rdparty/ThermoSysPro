...
algorithm
  ...
  if ((40 < T) and (T < 90)) then
    TT := T/273.16;
    Ps0 := 0.78614 + 10.7954*TT/(1 + TT) - 5.028*Modelica.Math.log10(1 + TT) + 1.50475e-5*(1. - 1./(10^(8.2969*TT)) + 0.00042873*(10^(4.76955*TT/(1. + TT))) - 1.);
    Ps := (10^Ps0)*100.;
    xs := 0.622*Ps/(Patm - Ps);
  end if;
  if (T >= 90) then
  ...
  annotation(
    ...
    Documentation(info = "
Problem: discontinuous function. Should be replaced by a continuous one.   
