...
algorithm
  der_mu := -2*Viscosity_cm2*temp^(-3)*der_temp - 1*Viscosity_cm1*temp^(-2)*der_temp + Viscosity_c1*der_temp + 2*Viscosity_c2*temp*der_temp + 3*Viscosity_c3*temp^2*der_temp + 4*Viscosity_c4*temp^3*der_temp;
  annotation(
