...
model StaticWallFlueGasesExchanger "Static wall/flue gases exchanger"
  ...
  Units.SI.Temperature T[Ns + 1](start = fill(900, Ns + 1)) "Flue gases temperature at the inlet of section i";
  Units.SI.Temperature Tm[Ns](start = fill(900, Ns)) "Average flue gases temperature in section i";
  Units.SI.SpecificEnthalpy h[Ns + 1](start = fill(1e6, Ns + 1)) "Flue gases specific enthalpy at the inlet of section i";
  Units.SI.Temperature Tp[Ns](start = fill(500, Ns)) "Wall temperature";
  Units.SI.AbsolutePressure Pe(start = 1e5) "Flue gases partial pressure at the inlet";
  ...
  Units.SI.CoefficientOfHeatTransfer Kcc[Ns](start = fill(0, Ns)) "Intermedaite variable for the computation of the convective heat exchange coefficient";
  Units.SI.CoefficientOfHeatTransfer Krr[Ns](start = fill(0, Ns)) "Intermedaite variable for the computation of the radiative heat exchange coefficient";
  Units.SI.Power dW[Ns](start = fill(0, Ns)) "Power exchange between the wall and the fluid in each section";
  Units.SI.Power W(start = 0) "Total power exchanged";
  ThermoSysPro.Units.SI.TemperatureDifference DeltaT[Ns](start = fill(50, Ns)) "Temperature difference between the fluid and the wall";
...
/* Total power exchanged */
  ...
  annotation(
    ...
    Icon(graphics = {Rectangle(extent = {{-100, 20}, {100, -20}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 0}, fillPattern = FillPattern.Backward), Line(points = {{-60, 20}, {-60, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{60, 20}, {60, -20}})}),
