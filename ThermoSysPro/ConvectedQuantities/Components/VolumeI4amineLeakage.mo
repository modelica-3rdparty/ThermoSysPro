...
model VolumeI4amineLeakage
  "Mixing volume with 4 inlets and 4 outlets"
...
public
  ...
  WaterSteam.Connectors.FluidInlet Ce4(redeclare package Species = Species)
    annotation (                             layer="icon", Placement(
  ...
  WaterSteam.Connectors.FluidOutlet Cs4(redeclare package Species = Species)
                           annotation (                           layer="icon",
  ...
  WaterSteam.Connectors.FluidOutlet Cs3(redeclare package Species = Species)
                           annotation (                            layer="icon",
      Placement(transformation(extent={{90,-90},{110,-70}}, rotation=0)));
  ThermoSysPro.ConvectedQuantities.Components.MassBalance4amineLeakage sub_massBalance(redeclare
      package Species =                                                                  Species,
   n_in=4, n_out=4,
   dynamic_mass_balance=dynamic_mass_balance,
   V=V,
   Qin = {Ce1.Q,Ce2.Q,Ce3.Q,Ce4.Q},
   Qout = {Cs1.Q,Cs2.Q,Cs3.Q,Cs4.Q},
   rho = rho,
   T=T,
   Qloss = Qloss.signal)
    annotation (Placement(transformation(extent={{-106,18},{-66,58}})));
  InstrumentationAndControl.Connectors.InputReal Qloss "Mass flow leakage [kg/s]" annotation (Placement(transformation(extent={{-13,-10.5},{13,10.5}},
          rotation=90,
        origin={61,111.5}),
...
equation
  ...
  sub_massBalance.mix_out.SubC = {Cs1.SubC,Cs2.SubC,Cs3.SubC,Cs4.SubC};
  ...
  if (cardinality(Ce4) == 0) then
    ...
    Ce4.h = 1.e5;
    Ce4.b = true;
    Ce4.SubC = fill(0,size(Ce4.SubC,1));
  end if;
