...
function scalarToColor_validityRange "Map a scalar to a color using a color map. colorMap used between T_min and T_nominal. Reversed colorMap is used between T_nominal and T_max"
...
/*Had to add 1e-7 and not Modelica.Constants.eps because otherwise a pulse would appear at the time where T=T_min or T_max (in the case where a ramp for T finishes at T_min or T_max*/
  color := noEvent(if (T < T_min - eps*1e-10 or T > T_max + eps*1e-10) then {138, 43, 226} elseif T_nominal == T_max then colorMap[1 + integer((size(colorMap, 1) - 1)*(max(T_min, min(T, T_nominal)) - T_min)/(T_nominal - T_min)), :]
   elseif T_nominal == T_min then colorMap[1 + integer((size(colorMap, 1) - 1)*(1 - (max(T_nominal, min(T, T_max)) - T_nominal)/(T_max - T_nominal))), :]
   elseif T < T_nominal then colorMap[1 + integer((size(colorMap, 1) - 1)*(max(T_min, min(T, T_nominal)) - T_min)/(T_nominal - T_min)), :]
   elseif T >= T_nominal then colorMap[1 + integer((size(colorMap, 1) - 1)*(1 - (max(T_nominal, min(T, T_max)) - T_nominal)/(T_max - T_nominal))), :] else {100, 100, 100});
