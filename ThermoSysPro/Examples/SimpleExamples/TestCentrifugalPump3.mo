...
model TestCentrifugalPump3
  ThermoSysPro.InstrumentationAndControl.Blocks.Logique.Pulse Pulse1(width = 200, period = 500) annotation(
    Placement(transformation(extent = {{-100, -60}, {-80, -40}}, rotation = 0)));
  ThermoSysPro.ElectroMechanics.Machines.SynchronousMotor Motor1(Im(start = 2000)) annotation(
    Placement(transformation(extent = {{-80, -80}, {-60, -60}}, rotation = 0)));
  ThermoSysPro.WaterSteam.Machines.CentrifugalPump centrifugalPump1(mode_car_hn = 1, mode_car_Cr = 1, mode_car = 1, dynamic_mech_equation = true, w_a(start = 1), C2(P(start = 300000))) annotation(
    Placement(transformation(extent = {{0, -40}, {-20, -20}}, rotation = 0)));
  WaterSteam.Volumes.Tank Tank(ze2 = 10, zs2 = 10) annotation(
  ...
  ThermoSysPro.WaterSteam.PressureLosses.LumpedStraightPipe lumpedStraightPipe(continuous_flow_reversal = true, inertia = true) annotation(
...
equation
  ...
  connect(Motor1.C, centrifugalPump1.M) annotation(
