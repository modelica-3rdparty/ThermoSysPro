...
model TestCentrifugalPump4
  ThermoSysPro.WaterSteam.Machines.CentrifugalPump centrifugalPump1(mode_car_hn = 1, mode_car_Cr = 1, mode_car = 1, dynamic_mech_equation = true, w_a(start = 1), C2(P(start = 297050)), pro(d(start = 997.375)), hn(start = 22), Pm(start = 189095)) annotation(
    Placement(transformation(extent = {{0, -40}, {-20, -20}}, rotation = 0)));
  ThermoSysPro.WaterSteam.Volumes.Tank Tank(ze2 = 10, zs2 = 10, Cs2(P(start = 296800))) annotation(
    Placement(transformation(extent = {{-20, 20}, {0, 40}}, rotation = 0)));
  ThermoSysPro.WaterSteam.PressureLosses.ControlValve Valve(Pm(start = 289870)) annotation(
  ...
  ThermoSysPro.WaterSteam.PressureLosses.LumpedStraightPipe lumpedStraightPipe(continuous_flow_reversal = true, inertia = true, C1(P(start = 269870)), Q(start = 498, fixed = false)) annotation(
    Placement(transformation(origin = {50, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  ThermoSysPro.ElectroMechanics.BoundaryConditions.SourceMechanicalPower sourceTorque annotation(
    Placement(transformation(extent = {{-60, -80}, {-40, -60}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Blocks.Sources.Pulse pulse(width = 200, period = 500, amplitude = 124000) annotation(
    Placement(transformation(extent = {{-100, -80}, {-80, -60}}, rotation = 0)));
equation
  connect(centrifugalPump1.C2, Tank.Ce2) annotation(
  ...
  connect(centrifugalPump1.C1, lumpedStraightPipe.C2) annotation(
    Line(points = {{0, -30}, {40, -30}}));
  connect(lumpedStraightPipe.C1, Valve.C2) annotation(
    Line(points = {{60, -30}, {80, -30}, {80, 24}, {60, 24}}));
  connect(pulse.y, sourceTorque.IPower) annotation(
    Line(points = {{-79, -70}, {-55, -70}}));
  connect(sourceTorque.M, centrifugalPump1.M) annotation(
    Line(points = {{-39, -70}, {-10, -70}, {-10, -41}}));
  annotation(
