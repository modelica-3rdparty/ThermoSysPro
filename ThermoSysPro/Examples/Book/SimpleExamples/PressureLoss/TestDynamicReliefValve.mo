...
model TestDynamicReliefValve
  ...
  ThermoSysPro.WaterSteam.PressureLosses.LumpedStraightPipe Pipe2(continuous_flow_reversal = true, Pm(start = 199283), Q(start = 1335.993983016864)) annotation(
    Placement(transformation(extent = {{40, -20}, {60, 0}}, rotation = 0)));
  ThermoSysPro.WaterSteam.BoundaryConditions.SinkP Sink2 annotation(
    Placement(transformation(extent = {{80, -20}, {100, 0}}, rotation = 0)));
  ThermoSysPro.WaterSteam.PressureLosses.LumpedStraightPipe Pipe1(continuous_flow_reversal = true, D = 0.4, Pm(start = 1478795.1075221882)) annotation(
    Placement(transformation(extent = {{-40, -20}, {-20, 0}}, rotation = 0)));
  ThermoSysPro.WaterSteam.Volumes.VolumeD VolumeD1(P(start = 298566), h(start = 72156.6431966866)) annotation(
    Placement(transformation(extent = {{0, -20}, {20, 0}}, rotation = 0)));
  ThermoSysPro.WaterSteam.BoundaryConditions.SinkP Sink1 annotation(
...
equation
  connect(Pipe2.C2, Sink2.C) annotation(
    Line(points = {{60, -10}, {80, -10}}, color = {0, 0, 255}));
  connect(Pipe1.C2, VolumeD1.Ce) annotation(
    Line(points = {{-20, -10}, {0, -10}}, color = {0, 0, 255}));
  connect(VolumeD1.Cs3, Pipe2.C1) annotation(
    Line(points = {{20, -10}, {40, -10}}, color = {0, 0, 255}));
  connect(ReliefValve.C1, VolumeD1.Cs1) annotation(
    Line(points = {{10, 20.2}, {10, 0}}));
  connect(ReliefValve.C2, Sink1.C) annotation(
    Line(points = {{20, 30}, {30, 30}, {30, 30}, {40, 30}}, color = {0, 0, 255}));
  connect(sourceP.C, Pipe1.C1) annotation(
    Line(points = {{-60, -10}, {-40, -10}}, color = {0, 0, 255}));
  connect(ramp.y, sourceP.IPressure) annotation(
    Line(points = {{-79, 30}, {-60, 30}, {-60, 8}, {-80, 8}, {-80, -10}, {-75, -10}}, color = {28, 108, 200}));
  annotation(
    Diagram(graphics, coordinateSystem(preserveAspectRatio = false, initialScale = 0.1)),
...
This model is documented in Sect. 13.13.5 of the ThermoSysPro book.  
