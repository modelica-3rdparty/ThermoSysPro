...
model HeatExchangerWallWithLosses "Heat exchanger wall"
  ...
  parameter Real Sc = 0.5 "Decrease factor of the external heat exchange surface for Solar Receiver or Boiler ";
  parameter Units.SI.ThermalConductivity lambda = 21 "Thermal conductivity of the pipes";
  parameter Real Eps = 0.6 "Tube emissivity";
  parameter Units.SI.Thickness e_ins = 0.1 "Insulation thickness";
  parameter Units.SI.ThermalConductivity lambda_ins = 0.035 "Equivalent thermal conductivity of Insulation + pipes";
  constant Units.SI.Acceleration g = Modelica.Constants.g_n "Gravity constant";
  parameter Real hcCorr1 = 1.00 "Corrective term for Forced convection losses coefficient for each node";
  parameter Real hcCorr2 = 1.00 "Corrective term for Natural convection losses coefficient for each node";
  ...
  Units.SI.ThermalConductivity lambda_air(start = 0.03) "Air thermal conductivity";
  ...
  Units.SI.CoefficientOfHeatTransfer hc_n[Ns](start = fill(5, Ns)) "Natural Convection losses coefficient";
  Units.SI.CoefficientOfHeatTransfer hc_f(start = 11) "Forced Convection losses coefficient";
  Units.SI.CoefficientOfHeatTransfer hc_f1(start = 11) "Forced Convection losses coefficient";
  Units.SI.CoefficientOfHeatTransfer hc_f2(start = 11) "Forced Convection losses coefficient";
  Units.SI.CoefficientOfHeatTransfer hc_f3(start = 11) "Forced Convection losses coefficient";
  Units.SI.CoefficientOfHeatTransfer hc[Ns](start = fill(5, Ns)) "Mixed Convection losses coefficient";
  ...
  InstrumentationAndControl.Connectors.InputReal AtmTemp "Atmospheric temperature (K)" annotation(
    Placement(transformation(extent = {{-120, 30}, {-100, 50}}, rotation = 0)));
  InstrumentationAndControl.Connectors.InputReal WindVelo "wind velocity" annotation(
    Placement(transformation(extent = {{-120, 64}, {-100, 84}}, rotation = 0)));
protected
...
/* Convection Losses coefficient (using the Siebers & Kraabel correlation) */
  hc_f1 = noEvent(if (Re < 7e5) and (Re > 1.e-6) then hcCorr1*lambda_air/D_rec*(0.3 + (0.488*Re^0.5*(1 + (Re/282000)^0.625)^0.8)) else if (Re < 2.2e7) and (Re > 7e5) then hcCorr1*lambda_air/D_rec*2.57e-3*Re^0.98 else if (Re >= 2.2e7) then hcCorr1*lambda_air/D_rec*0.0455*Re^0.81 else 1e-6);
  hc_f2 = noEvent(if (Re < 1.8e5) and (Re > 1.e-6) then hcCorr1*lambda_air/D_rec*(0.3 + (0.488*Re^0.5*(1 + (Re/282000)^0.625)^0.8)) else if (Re < 4e6) and (Re > 1.8e5) then hcCorr1*lambda_air/D_rec*13.5e-3*Re^0.89 else if (Re >= 4e6) then hcCorr1*lambda_air/D_rec*0.0455*Re^0.81 else 1e-6);
  hc_f3 = noEvent(if (Re < 1e5) and (Re > 1.e-6) then hcCorr1*lambda_air/D_rec*(0.3 + (0.488*Re^0.5*(1 + (Re/282000)^0.625)^0.8)) else if (Re >= 1e5) then hcCorr1*lambda_air/D_rec*0.0455*Re^0.81 else 1e-6);
  hc_f = noEvent(if (ksD < 150e-5) and (ksD > 1e-10) then hc_f1 else if (ksD < 700e-5) and (ksD >= 150e-5) then hc_f2 else hc_f3);
...
//WLosses [i] = WConv[i] + WRad[i];
  ...
  annotation(
    Icon(graphics = {Rectangle(extent = {{-100, 10}, {100, -10}}, lineColor = {0, 0, 0}, fillColor = {192, 192, 192}, fillPattern = FillPattern.CrossDiag), Text(extent = {{-80, 40}, {-20, 20}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "2"), Text(extent = {{-80, -20}, {-20, -40}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "1"), Text(extent = {{20, 40}, {98, 20}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "External"), Text(extent = {{20, -20}, {98, -40}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "Internal"), Text(extent = {{-94, 62}, {-20, 40}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "With Losses"), Text(extent = {{-122, 32}, {-100, 16}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "Tatm"), Text(extent = {{-122, 70}, {-96, 50}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "v_wind")}),
    Diagram(graphics = {Rectangle(extent = {{-100, 10}, {100, -10}}, lineColor = {0, 0, 0}, fillColor = {192, 192, 192}, fillPattern = FillPattern.Solid), Text(extent = {{-80, 30}, {-20, 10}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "Side 2"), Text(extent = {{-80, -10}, {-20, -30}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "Side 1"), Text(extent = {{20, -10}, {98, -30}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "Internal"), Text(extent = {{20, 30}, {98, 10}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "External"), Text(extent = {{-96, 66}, {-22, 44}}, lineColor = {0, 0, 255}, fillColor = {255, 170, 170}, fillPattern = FillPattern.Solid, textString = "With Losses")}),
    DymolaStoredErrors,
    Documentation(info = "
