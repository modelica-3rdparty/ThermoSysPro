...
model StaticWallFlueGasesExchanger "Static wall - flue gases exchanger"
  ...
  parameter Units.SI.Area Surf_ext = pi*Dext*Ls*NbTub*CSailettes "Heat exchange surface for one section";
  ...
  parameter Units.SI.Temperature Tp0 = 500 "Wall temperature (active if the thermal connector is not connected)";
  ...
  Units.SI.Temperature Tp[N - 1](start = fill(500, N - 1)) "Wall temperature in thermal node i";
  Units.SI.AbsolutePressure Pco2[N - 1] "CO2 partial pressure in thermal node i";
  Units.SI.AbsolutePressure Ph2o[N - 1] "H2O partial pressure in thermal node i";
  ...
  Units.SI.CoefficientOfHeatTransfer K(start = 0) "Total heat exchange coefficient";
  Units.SI.CoefficientOfHeatTransfer Kc(start = 0) "Convective heat exchange coefficient";
  Units.SI.CoefficientOfHeatTransfer Kr(start = 0) "Radiative heat exchange coefficient";
  Units.SI.CoefficientOfHeatTransfer Kcc[N - 1](start = fill(0, N - 1)) "Intermediate variable for the computation of the convective heat exchange coefficient";
  Units.SI.CoefficientOfHeatTransfer Krr[N - 1](start = fill(0, N - 1)) "Intermediate variable for the computation of the radiative heat exchange coefficient";
  Units.SI.Power dW1[N - 1](start = fill(0, N - 1)) "Power exchange between the wall and the fluid in each thermal node";
  Units.SI.Power W(start = 0) "Total power exchanged";
  ThermoSysPro.Units.SI.TemperatureDifference deltaT[N - 1](start = fill(50, N - 1)) "Temperature difference between the fluid and the wall";
...
protected
  ...
  parameter Units.SI.Length Deq = 4*Sgaz/Perb "Equivalent diameter for longitudinal flux";
  parameter Units.SI.Length Perb = Surf_ext/Ls "Geometrical parameter";
  parameter Units.SI.CoefficientOfHeatTransfer Kdef = 50 "Heat exchange coefficient in case of zero flow";
...
/* Total fluid diffusion resistance */
  ...
  annotation(
    Diagram(graphics = {Rectangle(extent = {{-100, 20}, {100, -20}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 0}, fillPattern = FillPattern.Backward), Line(points = {{-60, 20}, {-60, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{60, 20}, {60, -20}})}),
    Icon(graphics = {Rectangle(extent = {{-100, 20}, {100, -20}}, lineColor = {0, 0, 255}, fillColor = DynamicSelect({255, 255, 0}, if diffusion then fill_color_singular else fill_color_static), fillPattern = FillPattern.Backward), Line(points = {{-60, 20}, {-60, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{60, 20}, {60, -20}})}),
