...
model StaticExchangerKS "Static heat exchanger with fixed K and S (Coefficient of heat transfer and Heat exchanger surface)"
  ...
  parameter Units.SI.CoefficientOfHeatTransfer K = 100 "Global heat exchange coefficient";
  ...
  Units.SI.Temperature Tec(start = 400) "Temperature of the hot fluid at the inlet";
  Units.SI.Temperature Tsc(start = 300) "Temperature of the hot fluid at the outlet";
  Units.SI.Temperature Tef(start = 300) "Temperature of the cold fluid at the inlet";
  Units.SI.Temperature Tsf(start = 400) "Temperature of the cold fluid at the outlet";
  ...
  Units.SI.SpecificHeatCapacity Cpf(start = 2000) "Specific heat capacity of the cold fluid";
  Units.SI.SpecificHeatCapacity Cpc(start = 2000) "Specific heat capacity of the hot fluid";
  ...
  Interfaces.Connectors.FluidOutlet Sc annotation(
    Placement(transformation(extent = {{90, -40}, {110, -20}}, rotation = 0), iconTransformation(extent = {{90, -40}, {110, -20}})));
  Interfaces.Connectors.FluidInlet Ec annotation(
    Placement(transformation(extent = {{-110, -40}, {-90, -20}}, rotation = 0), iconTransformation(extent = {{-110, -40}, {-90, -20}})));
  Interfaces.Connectors.FluidInlet Ef annotation(
    Placement(transformation(extent = {{-110, 20}, {-90, 40}}, rotation = 0), iconTransformation(extent = {{-110, 20}, {-90, 40}})));
  Interfaces.Connectors.FluidOutlet Sf annotation(
    Placement(transformation(extent = {{90, 20}, {110, 40}}, rotation = 0), iconTransformation(extent = {{90, 20}, {110, 40}})));
  InstrumentationAndControl.Connectors.InputReal Kcorr "K_correction" annotation(
...
/* Co-current exchanger */
  ...
  DT2 = if (exchanger_conf == 1) then DT1*Modelica.Math.exp(-Kcor*K*S*(1/(Qc*Cpc) - 1/(Qf*Cpf))) else DT1*Modelica.Math.exp(-Kcor*K*S*(1/(Qc*Cpc) + 1/(Qf*Cpf)));
/* Power exchanged */
  W = Qc*(Ec.h - Sc.h);
  W = Qf*(Sf.h - Ef.h);
