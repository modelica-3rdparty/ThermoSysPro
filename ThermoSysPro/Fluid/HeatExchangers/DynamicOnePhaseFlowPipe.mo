...
model DynamicOnePhaseFlowPipe "Dynamic one-phase flow pipe"
  ...
  parameter Units.SI.AbsolutePressure P0 = 1.e5 "Fluid initial pressure (active if steady_state = false)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state));
  parameter Units.SI.Temperature T0[Ns] = fill(290, Ns) "Initial fluid temperature (active if steady_state = false and option_temperature=true)" annotation(
    Evaluate = true,
  ...
  Units.SI.Power dW1[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Thermal power exchanged on the water side for node i";
  ...
  Units.SI.CoefficientOfHeatTransfer hc[N - 1](start = fill(2000, N - 1), nominal = fill(200, N - 1)) "Fluid heat exchange coefficient in node i";
  Units.SI.ReynoldsNumber Re1[N - 1](start = fill(6.e4, N - 1), nominal = fill(0.5e4, N - 1)) "Fluid Reynolds number in thermal node i";
  Units.SI.ReynoldsNumber Re2[N](start = fill(6.e4, N), nominal = fill(0.5e4, N)) "Fluid Reynolds number in hydraulic node i";
  Real Pr[N - 1](start = fill(4, N - 1), nominal = fill(1, N - 1)) "Fluid Prandtl number in node i";
  Units.SI.ThermalConductivity k1[N - 1](start = fill(0.6, N - 1), nominal = fill(0.6, N - 1)) "Fluid thermal conductivity in thermal node i";
  Units.SI.ThermalConductivity k2[N](start = fill(0.6, N), nominal = fill(0.6, N)) "Fluid thermal conductivity in hydraulic node i";
  Units.SI.DynamicViscosity mu1[N - 1](start = fill(2.e-4, N - 1), nominal = fill(2.e-4, N - 1)) "Fluid dynamic viscosity in thermal node i";
  Units.SI.DynamicViscosity mu2[N](start = fill(2.e-4, N), nominal = fill(2.e-4, N)) "Fluid dynamic viscosity in hydraulic node i";
  Units.SI.SpecificHeatCapacity cp1[N - 1](start = fill(4000, N - 1), nominal = fill(4000, N - 1)) "Fluid specific heat capacity in thermal node i";
  Units.SI.SpecificHeatCapacity cp2[N](start = fill(4000, N), nominal = fill(4000, N)) "Fluid specific heat capacity in hydraulic node i";
...
protected
  ...
  parameter Units.SI.Area dSi = pi*Di*dx1*CSailettes "Internal heat exchange area for a node";
...
/* Total fluid diffusion resistance */
  ...
  annotation(
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Line(points = {{-60, 20}, {-60, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{60, 20}, {60, -20}}), Line(points = {{-60, 20}, {-60, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{60, 20}, {60, -20}}), Rectangle(extent = {{-100, 20}, {100, -20}}, lineColor = {28, 108, 200}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid), Line(points = {{60, 20}, {60, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{-60, 20}, {-60, -20}})}),
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Rectangle(extent = {{-100, 20}, {100, -20}}, lineColor = {0, 0, 255}, fillColor = DynamicSelect({85, 170, 255}, if dynamic_energy_balance then fill_color_dynamic else if diffusion then fill_color_singular else fill_color_static), fillPattern = DynamicSelect(FillPattern.Solid, if dynamic_mass_balance and dynamic_energy_balance then FillPattern.Sphere else FillPattern.Solid)), Line(points = {{-60, 20}, {-60, -20}}), Line(points = {{-20, 20}, {-20, -20}}), Line(points = {{20, 20}, {20, -20}}), Line(points = {{60, 20}, {60, -20}})}),
