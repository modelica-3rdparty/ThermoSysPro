...
model DynamicTwoPhaseFlowRiser "Dynamic two-phase flow riser"
  ...
  parameter Units.SI.Temperature T0[Ns] = fill(300, Ns) "Initial fluid temperature (active if steady_state=false and option_temperature=true)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state and option_temperature));
  parameter Units.SI.SpecificEnthalpy h0[Ns] = fill(1e5, Ns) "Initial fluid specific enthalpy (active if steady_state=false and option_temperature=false)" annotation(
    Evaluate = true,
  ...
  parameter Boolean advection = false "true: momentum balance equation with advection term - false: without advection term";
  parameter Boolean dynamic_energy_balance = true "true: dynamic energy balance equation - false: static energy balance equation";
  parameter Boolean dynamic_mass_balance = true "true: dynamic mass balance equation - false: static mass balance equation (active if dynamic_energy_balance=true)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance));
  parameter Boolean simplified_dynamic_energy_balance = true "true: simplified dynamic energy balance equation - false: full dynamic energy balance equation (active if dynamic_energy_balance=true and dynamic_mass_balance=true)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance and dynamic_mass_balance));
  ...
  Units.SI.AbsolutePressure P[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e5, N + 1)) "Fluid pressure in node i";
  Units.SI.MassFlowRate Q[N](start = fill(10, N), nominal = fill(10, N)) "Mass flow rate in node i";
  Units.SI.SpecificEnthalpy h[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e6, N + 1)) "Fluid specific enthalpy in node i";
  Units.SI.SpecificEnthalpy hb[N] "Fluid specific enthalpy at the boundary of node i";
  Units.SI.AbsolutePressure Pb[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e5, N + 1)) "Bounded fluid pressure in node i";
  Units.SI.Density rho1[N - 1](start = fill(998, N - 1), nominal = fill(1, N - 1)) "Fluid density in thermal node i";
  Units.SI.Density rho2[N](start = fill(998, N), nominal = fill(1, N)) "Fluid density in hydraulic node i";
  Units.SI.Density rhoc[N + 1](start = fill(998, N + 1), nominal = fill(1, N + 1)) "Fluid density at the boudary of node i";
  Units.SI.MassFlowRate BQ[N - 1] "Right hand side of the mass balance equation for thermal node i";
  Units.SI.Power BH[N - 1] "Right hand side of the energy balance equation for thermal node i";
  Units.SI.Power dW1_conv[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Convection thermal power exchanged on the liquid side for node i";
  Units.SI.Power dW1_diff[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Diffusion thermal power exchanged on the liquid side for node i";
  Units.SI.Power dW1[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Thermal power exchanged on the liquid side for node i";
  Units.SI.Power dW2_conv[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Convection thermal power exchanged on the liquid side for node i";
  Units.SI.Power dW2_diff[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Diffusion thermal power exchanged on the liquid side for node i";
  Units.SI.Power dW2[N - 1](start = fill(3.e5, N - 1), nominal = fill(3.e5, N - 1)) "Thermal power exchanged on the liquid side for node i";
  Units.SI.Power W1t "Total power exchanged on the liquid side";
  Units.SI.Temperature Tp1[N - 1](each start = 500.0) "Wall temperature in node i";
  Units.SI.Temperature Tp2[N - 1](each start = 500.0) "Wall temperature in node i";
  Units.SI.CoefficientOfHeatTransfer hi[N - 1](start = fill(2000, N - 1), nominal = fill(2.e4, N - 1)) "Fluid heat exchange coefficient in node i";
  Units.SI.CoefficientOfHeatTransfer hcl[N - 1](start = fill(2000, N - 1), nominal = fill(200, N - 1)) "Fluid heat exchange coefficient in node i for the liquid fraction";
  Units.SI.CoefficientOfHeatTransfer hcv[N - 1](start = fill(0, N - 1), nominal = fill(200, N - 1)) "Fluid heat exchange coefficient in node i for the vapor fraction";
  Real S[N - 1] "Corrective terme correctif for nucleation removal";
  Real E[N - 1] "Corrective term for hcl";
  Units.SI.CoefficientOfHeatTransfer heb[N - 1](start = fill(0, N - 1), nominal = fill(5.e5, N - 1)) "Fluid heat exchange coefficient for vaporization in thermal node i";
  Units.SI.ReynoldsNumber Rel1[N - 1](start = fill(6.e4, N - 1), nominal = fill(0.5e4, N - 1)) "Reynolds number in thermal node i for the liquid";
  Units.SI.ReynoldsNumber Rel2[N](start = fill(6.e4, N), nominal = fill(0.5e4, N)) "Reynolds number in hydraulic node i for the liquid";
  Units.SI.ReynoldsNumber Rev1[N - 1](start = fill(0.1e4, N - 1), nominal = fill(5.e5, N - 1)) "Reynolds number in thermal node i for the vapor";
  Units.SI.ReynoldsNumber Rev2[N](start = fill(0.1e4, N), nominal = fill(5.e5, N)) "Reynolds number in hydraulic node i for the vapor";
  ...
  Units.SI.ThermalConductivity kl1[N - 1](start = fill(0.6, N - 1), nominal = fill(0.6, N - 1)) "Thermal conductivity in thermal node i for the liquid";
  Units.SI.ThermalConductivity kl2[N](start = fill(0.6, N), nominal = fill(0.6, N)) "Thermal conductivity in hydraulic node i for the liquid";
  Units.SI.ThermalConductivity kv1[N - 1](start = fill(0.03, N - 1), nominal = fill(0.03, N - 1)) "Thermal conductivity in thermal node i for the vapor";
  Units.SI.ThermalConductivity kv2[N](start = fill(0.03, N), nominal = fill(0.03, N)) "Thermal conductivity in hydraulic node i for the vapor";
  ...
  Units.SI.DynamicViscosity mul1[N - 1](start = fill(2.e-4, N - 1), nominal = fill(2.e-4, N - 1)) "Dynamic viscosity in thermal node i for the liquid";
  Units.SI.DynamicViscosity mul2[N](start = fill(2.e-4, N), nominal = fill(2.e-4, N)) "Dynamic viscosity in hydraulic node i for the liquid";
  Units.SI.DynamicViscosity muv1[N - 1](start = fill(1.e-5, N - 1), nominal = fill(1.e-4, N - 1)) "Dynamic viscosity in thermal node i for the vapor";
  Units.SI.DynamicViscosity muv2[N](start = fill(1.e-5, N), nominal = fill(1.e-4, N)) "Dynamic viscosity in hydraulic node i for the vapor";
  Units.SI.SpecificHeatCapacity cpl1[N - 1](start = fill(4000, N - 1), nominal = fill(4000, N - 1)) "Specific heat capacity in thermal node i for the liquid";
  Units.SI.SpecificHeatCapacity cpl2[N](start = fill(4000, N), nominal = fill(4000, N)) "Specific heat capacity in hydraulic node i for the liquid";
  Units.SI.SpecificHeatCapacity cpv1[N - 1](start = fill(2000, N - 1), nominal = fill(2000, N - 1)) "Specific heat capacity in thermal node i for the vapor";
  Units.SI.SpecificHeatCapacity cpv2[N](start = fill(4000, N), nominal = fill(4000, N)) "Specific heat capacity in hydraulic node i for the vapor";
  Real Bo[N - 1](start = fill(0, N - 1), nominal = fill(4.e-4, N - 1)) "Boiling number";
  Real Xtt[N - 1](start = fill(1, N - 1), nominal = fill(1, N - 1)) "Martinelli number";
  Units.SI.SpecificEnthalpy lv[N - 1](start = fill(2.e6, N - 1), nominal = fill(2.e6, N - 1)) "Specific enthalpy for vaporisation";
  Units.SI.Density rhol1[N - 1](start = fill(998, N - 1), nominal = fill(998, N - 1)) "Fluid density in thermal node i for the liquid";
  Units.SI.Density rhol2[N](start = fill(998, N), nominal = fill(998, N)) "Fluid density in hydraulic node i for the liquid";
  Units.SI.Density rhov1[N - 1](start = fill(1, N - 1), nominal = fill(1, N - 1)) "Fluid density in thermal node i for the vapor";
  Units.SI.Density rhov2[N](start = fill(1, N), nominal = fill(1, N)) "Fluid density in hydraulic node i for the vapor";
  ...
  ThermoSysPro.Units.SI.PressureDifference dpa[N] "Advection term for the mass balance equation in node i";
  ThermoSysPro.Units.SI.PressureDifference dpf[N] "Friction pressure loss in node i";
  ThermoSysPro.Units.SI.PressureDifference dpg[N] "Gravity pressure loss in node i";
  ...
  Units.SI.MassFlowRate gamma[N] "Total diffusion conductance in hydraulic node i";
  Units.SI.MassFlowRate gamma_e[N - 1] "Diffusion conductance at inlet of thermal node i";
  Units.SI.MassFlowRate gamma_s[N - 1] "Diffusion conductance at outlet of thermal node i";
  Units.SI.Power Je[N - 1] "Thermal power diffusion from inlet of thermal node i";
  Units.SI.Power Js[N - 1] "Thermal power diffusion from outlet of thermal node i";
  ...
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph pro1[N - 1] annotation(
    Placement(transformation(extent = {{-100, 80}, {-80, 100}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph proc[2] annotation(
    Placement(transformation(extent = {{80, 80}, {100, 100}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph pro2[N] annotation(
    Placement(transformation(extent = {{-60, 80}, {-40, 100}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat vsat2[N] annotation(
    Placement(transformation(extent = {{80, -100}, {100, -80}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat lsat2[N] annotation(
    Placement(transformation(extent = {{40, -100}, {60, -80}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat vsat1[N - 1] annotation(
    Placement(transformation(extent = {{-60, -100}, {-40, -80}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat lsat1[N - 1] annotation(
    Placement(transformation(extent = {{-100, -100}, {-80, -80}}, rotation = 0)));
  Interfaces.Connectors.FluidInlet C1 annotation(
    Placement(transformation(extent = {{-110, 0}, {-90, 20}}, rotation = 0)));
  ThermoSysPro.Thermal.Connectors.ThermalPort CTh1[Ns] annotation(
    Placement(transformation(extent = {{-10, 60}, {10, 80}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet C2 annotation(
    Placement(transformation(extent = {{90, 0}, {110, 20}}, rotation = 0)));
...
protected
  ...
  parameter Units.SI.Area A = ntubes*pi*D^2/4 "Internal cross sectional pipe area";
  ...
  parameter Units.SI.Area dSi = pi*Di*dx1 "Internal heat exchange area for a node";
  ...
  parameter Units.SI.AbsolutePressure ptriple = ThermoSysPro.Properties.WaterSteam.BaseIF97.triple.ptriple "Triple point pressure";
