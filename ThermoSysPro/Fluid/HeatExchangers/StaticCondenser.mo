...
model StaticCondenser "Static condenser"
  ...
  parameter IF97Region region_ee = IF97Region.All_regions "IF97 region at the water inlet (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_se = IF97Region.All_regions "IF97 region at the water outlet (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_ex = IF97Region.All_regions "IF97 region at the extraction point (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
