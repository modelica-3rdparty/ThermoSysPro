...
equation
  ...
  for i in 1:N loop
    ...
    if noEvent((P[i+1] > pcrit) or (T2[i] > Tcrit)) then
      rhol2[i] = rho2[i];
      rhov2[i] = rho2[i];
      cpl2[i]  = Medium.specificHeatCapacityCp(state2[i]);
      cpv2[i]  = Medium.specificHeatCapacityCp(state2[i]);
    else
      rhol2[i] = max(rho2[i], Medium.bubbleDensity(sat2[i]));
      rhov2[i] = min(rho2[i], Medium.dewDensity(sat2[i]));
      cpl2[i]  = if noEvent(xv2[i] <= 0.0) then Medium.specificHeatCapacityCp(state2[i]) else Medium.specificHeatCapacityCp(Medium.setBubbleState(sat2[i]));
      cpv2[i]  = if noEvent(xv2[i] >= 1.0) then Medium.specificHeatCapacityCp(state2[i]) else Medium.specificHeatCapacityCp(Medium.setDewState(sat2[i]));
    end if;
