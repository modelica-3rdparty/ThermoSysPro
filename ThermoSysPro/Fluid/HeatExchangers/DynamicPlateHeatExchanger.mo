...
model DynamicPlateHeatExchanger "Dynamic plate heat exchanger"
  ...
  parameter Units.SI.MassFlowRate gamma_diff_c = 1e-4 "Diffusion conductance for the hot fluid (active if diffusion=true in neighbouring volumes)";
  parameter Units.SI.MassFlowRate gamma_diff_f = 1e-4 "Diffusion conductance for the cold fluid (active if diffusion=true in neighbouring volumes)";
  parameter Units.SI.AbsolutePressure Pc0 = 1.e5 "Hot fluid initial pressure (active if steady_state = false)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state));
  parameter Units.SI.AbsolutePressure Pf0 = 1.e5 "Cold fluid initial pressure (active if steady_state = false)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state));
  parameter Units.SI.Temperature Tc0[Ns] = fill(290, Ns) "Initial hot fluid temperature (active if steady_state = false and option_temperature=true)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state and option_temperature));
  parameter Units.SI.Temperature Tf0[Ns] = fill(290, Ns) "Initial cold fluid temperature (active if steady_state = false and option_temperature=true)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state and option_temperature));
  parameter Units.SI.SpecificEnthalpy hc0[Ns] = fill(1e5, Ns) "Initial hot fluid specific enthalpy (active if steady_state = false and option_temperature=false)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state and not option_temperature));
  parameter Units.SI.SpecificEnthalpy hf0[Ns] = fill(1e5, Ns) "Initial cold fluid specific enthalpy (active if steady_state = false and option_temperature=false)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state and not option_temperature));
  parameter Boolean dynamic_energy_balance = true "true: dynamic energy balance equation - false: static energy balance equation";
  parameter Boolean steady_state = true "true: start from steady state - false: start from T0 (if option_temperature = true) or h0 (if option_temperature=false)(active if dynamic_energy_balance=true)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance));
  parameter Boolean option_temperature = true "true: initial temperature is fixed - false: initial specific enthalpy is fixed (active if steady_state=false)" annotation(
    Evaluate = true,
    Dialog(enable = not steady_state));
  ...
  parameter Units.SI.Density p_rhoc = 0 "If > 0, fixed fluid density for the hot fluid" annotation(
    Evaluate = true,
    Dialog(tab = "Fluid", group = "Fluid properties"));
  parameter Units.SI.Density p_rhof = 0 "If > 0, fixed fluid density for the cold fluid" annotation(
    Evaluate = true,
    Dialog(tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_c = IF97Region.All_regions "Hot fluid IF97 region (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype_c == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_f = IF97Region.All_regions "Cold fluid IF97 region (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype_f == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  Units.SI.AbsolutePressure Pc[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e5, N + 1)) "Hot fluid pressure in node i";
  Units.SI.AbsolutePressure Pf[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e5, N + 1)) "Cold fluid pressure in node i";
  Units.SI.MassFlowRate Qc[N](start = fill(10, N), nominal = fill(10, N)) "Hot fluid mass flow rate in node i";
  Units.SI.MassFlowRate Qf[N](start = fill(10, N), nominal = fill(10, N)) "Cold fluid mass flow rate in node i";
  Units.SI.SpecificEnthalpy hc[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e6, N + 1)) "Hot fluid specific enthalpy in node i";
  Units.SI.SpecificEnthalpy hf[N + 1](start = fill(1.e5, N + 1), nominal = fill(1.e6, N + 1)) "Cold fluid specific enthalpy in node i";
  Units.SI.SpecificEnthalpy hbc[N] "Hot fluid specific enthalpy at the boundary of node i";
  Units.SI.SpecificEnthalpy hbf[N] "Cold fluid specific enthalpy at the boundary of node i";
  Units.SI.Density rhoc1[N - 1](start = fill(998, N - 1), nominal = fill(1, N - 1)) "Hot fluid density in thermal node i";
  Units.SI.Density rhof1[N - 1](start = fill(998, N - 1), nominal = fill(1, N - 1)) "Cold fluid density in thermal node i";
  Units.SI.Density rhoc2[N](start = fill(998, N), nominal = fill(1, N)) "Hot fluid density in hydraulic node i";
  Units.SI.Density rhof2[N](start = fill(998, N), nominal = fill(1, N)) "Cold fluid density in hydraulic node i";
  Units.SI.MassFlowRate BQc[N - 1] "Right hand side of the mass balance equation for thermal node i for the hot fluid";
  Units.SI.MassFlowRate BQf[N - 1] "Right hand side of the mass balance equation for thermal node i for the cold fluid";
  Units.SI.Power BHc[N - 1] "Right hand side of the energy balance equation for thermal node i for the cold fluid";
  Units.SI.Power BHf[N - 1] "Right hand side of the energy balance equation for thermal node i for the hot fluid";
  Units.SI.CoefficientOfHeatTransfer hconv[N - 1] "Global heat transfer coefficient in thermal node i";
  Units.SI.CoefficientOfHeatTransfer hconv_c[N - 1](start = fill(2000, N - 1), nominal = fill(200, N - 1)) "Hot fluid heat exchange coefficient in node i";
  Units.SI.CoefficientOfHeatTransfer hconv_f[N - 1](start = fill(2000, N - 1), nominal = fill(200, N - 1)) "Cold fluid heat exchange coefficient in node i";
  Units.SI.ThermalConductivity kc1[N - 1](start = fill(0.6, N - 1), nominal = fill(0.6, N - 1)) "Hot fluid thermal conductivity in thermal node i";
  Units.SI.ThermalConductivity kf1[N - 1](start = fill(0.6, N - 1), nominal = fill(0.6, N - 1)) "Cold fluid thermal conductivity in thermal node i";
  Units.SI.ThermalConductivity kc2[N](start = fill(0.6, N), nominal = fill(0.6, N)) "Hot fluid thermal conductivity in hydraulic node i";
  Units.SI.ThermalConductivity kf2[N](start = fill(0.6, N), nominal = fill(0.6, N)) "Cold fluid thermal conductivity in hydraulic node i";
  Units.SI.DynamicViscosity muc1[N - 1](start = fill(2.e-4, N - 1), nominal = fill(2.e-4, N - 1)) "Hot fluid dynamic viscosity in thermal node i";
  Units.SI.DynamicViscosity muf1[N - 1](start = fill(2.e-4, N - 1), nominal = fill(2.e-4, N - 1)) "Cold fluid dynamic viscosity in thermal node i";
  Units.SI.DynamicViscosity muc2[N](start = fill(2.e-4, N), nominal = fill(2.e-4, N)) "Hot fluid dynamic viscosity in hydraulic node i";
  Units.SI.DynamicViscosity muf2[N](start = fill(2.e-4, N), nominal = fill(2.e-4, N)) "Cold fluid dynamic viscosity in hydraulic node i";
  Units.SI.SpecificHeatCapacity cpc1[N - 1](start = fill(4000, N - 1), nominal = fill(4000, N - 1)) "Hot fluid specific heat capacity in thermal node i";
  Units.SI.SpecificHeatCapacity cpf1[N - 1](start = fill(4000, N - 1), nominal = fill(4000, N - 1)) "Cold fluid specific heat capacity in thermal node i";
  Units.SI.SpecificHeatCapacity cpc2[N](start = fill(4000, N), nominal = fill(4000, N)) "Hot fluid specific heat capacity in hydraulic node i";
  Units.SI.SpecificHeatCapacity cpf2[N](start = fill(4000, N), nominal = fill(4000, N)) "Cold fluid specific heat capacity in hydraulic node i";
  Units.SI.ThermalConductivity lambdac1[N - 1](start = fill(0.602698, N - 1)) "Hot fluid thermal conductivity in thermal node i";
  Units.SI.ThermalConductivity lambdaf1[N - 1](start = fill(0.597928, N - 1)) "Cold fluid thermal conductivity in thermal node i";
  ...
  Units.SI.Power dW[N - 1] "Thermal power exchanged between the hot and cold sides in thermal node i";
  Real qmc1[N - 1];
  Real qmf1[N - 1];
  Real qmc2[N];
  Real qmf2[N];
  ...
  Real quf[N];
  Real M;
  ThermoSysPro.Units.SI.PressureDifference DPc[N] "Pressure loss of the hot fluid in hydraulic node i";
  ThermoSysPro.Units.SI.PressureDifference DPf[N] "Pressure loss of the cold fluid in hydraulic node i";
  ...
  Units.SI.MassFlowRate gamma_c[N] "Total diffusion conductance in hydraulic node i for the hot fluid";
  Units.SI.MassFlowRate gamma_f[N] "Total diffusion conductance in hydraulic node i for the cold fluid";
  Units.SI.MassFlowRate gamma_e_c[N - 1] "Diffusion conductance at inlet of thermal node i for the hot fluid";
  Units.SI.MassFlowRate gamma_e_f[N - 1] "Diffusion conductance at inlet of thermal node i for the cold fluid";
  Units.SI.MassFlowRate gamma_s_c[N - 1] "Diffusion conductance at outlet of thermal node i for the hot fluid";
  Units.SI.MassFlowRate gamma_s_f[N - 1] "Diffusion conductance at outlet of thermal node i for the cold fluid";
  Units.SI.Power Je_c[N - 1] "Thermal power diffusion from inlet of thermal node i for the hot fluid";
  Units.SI.Power Je_f[N - 1] "Thermal power diffusion from inlet of thermal node i for the cold fluid";
  Units.SI.Power Js_c[N - 1] "Thermal power diffusion from outlet of thermal node i for the hot fluid";
  Units.SI.Power Js_f[N - 1] "Thermal power diffusion from outlet of thermal node i for the cold fluid";
  Units.SI.Power J_c[N - 1] "Total thermal power diffusion of thermal node i for the hot fluid";
  Units.SI.Power J_f[N - 1] "Total thermal power diffusion of thermal node i for the cold fluid";
  ...
  Interfaces.Connectors.FluidInlet Ec annotation(
    Placement(transformation(extent = {{-110, -10}, {-90, 10}}, rotation = 0)));
  Interfaces.Connectors.FluidInlet Ef annotation(
    Placement(transformation(extent = {{-60, -70}, {-40, -50}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet Sf annotation(
    Placement(transformation(extent = {{40, -70}, {60, -50}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet Sc annotation(
...
protected
  ...
  parameter Units.SI.Area Ac = 1 "Internal cross sectional pipe area for the hot fluid";
  parameter Units.SI.Area Af = 1 "Internal cross sectional pipe area for the cold fluid";
  parameter Units.SI.PathLength dx2_c = Vc/N/Ac "Length of a hydraulic node for the hot fluid";
  parameter Units.SI.PathLength dx2_f = Vf/N/Af "Length of a hydraulic node for the cold fluid";
...
/* Total fluid diffusion resistance */
  ...
  annotation(
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Rectangle(extent = {{-100, 60}, {100, -60}}, lineColor = {0, 0, 0}, fillPattern = FillPattern.Solid, fillColor = {127, 255, 0}), Line(points = {{-80, 60}, {-80, -60}}), Line(points = {{80, 60}, {80, -60}}), Line(points = {{-80, 0}, {-60, 0}, {-40, 20}, {40, -20}, {60, 0}, {80, 0}}, color = {28, 108, 200}), Line(points = {{-40, 60}, {-40, -60}}, color = {28, 108, 200}, pattern = LinePattern.Dot), Line(points = {{0, 60}, {0, -60}}, color = {28, 108, 200}, pattern = LinePattern.Dot), Line(points = {{40, 60}, {40, -60}}, color = {28, 108, 200}, pattern = LinePattern.Dot)}),
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Rectangle(extent = {{-100, 60}, {100, -60}}, lineColor = {0, 0, 0}, fillPattern = FillPattern.Solid, fillColor = DynamicSelect({85, 170, 255}, if dynamic_energy_balance then fill_color_dynamic else if diffusion then fill_color_singular else fill_color_static), lineThickness = 0), Line(points = {{-80, 60}, {-80, -60}}), Line(points = {{80, 60}, {80, -60}}), Line(points = {{-80, 0}, {-60, 0}, {-40, 20}, {40, -20}, {60, 0}, {80, 0}}, color = {28, 108, 200}), Line(points = {{-40, 60}, {-40, -60}}, color = {28, 108, 200}, pattern = LinePattern.Dot), Line(points = {{0, 60}, {0, -60}}, color = {28, 108, 200}, pattern = LinePattern.Dot), Line(points = {{40, 60}, {40, -60}}, color = {28, 108, 200}, pattern = LinePattern.Dot), Text(extent = {{104, 24}, {128, 10}}, lineColor = {0, 0, 255}, fillColor = {255, 213, 170}, fillPattern = FillPattern.Solid, textString = "Hot outlet"), Text(extent = {{-82, -66}, {-62, -76}}, lineColor = {0, 0, 255}, fillColor = {255, 213, 170}, fillPattern = FillPattern.Solid, textString = "Cold inlet"), Text(extent = {{-126, 24}, {-106, 14}}, lineColor = {0, 0, 255}, fillColor = {255, 213, 170}, fillPattern = FillPattern.Solid, textString = "Hot inlet"), Text(extent = {{66, -66}, {90, -76}}, lineColor = {0, 0, 255}, fillColor = {255, 213, 170}, fillPattern = FillPattern.Solid, textString = "Cold outlet")}),
    Window(x = 0.18, y = 0.05, width = 0.68, height = 0.94),
    Documentation(info = "
