...
model DynamicWaterHeaterOnePipe "Dynamic water heater"
  ...
  parameter Units.SI.Length PasL = 0.03 "Longitudianl step or Length bottom pipes triangular step";
  ...
  parameter Boolean steady_state = true "true: start from steady state - false: start from (Vf0, P0c, T0 or h0) (active if dynamic_energy_balance=true)" annotation(
    Evaluate = true,
  ...
  parameter Units.SI.AbsolutePressure P0c = 1.e5 "Fluid initial pressure (active if dynamic_energy_balance=true and steady_state=false)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance and not steady_state));
  parameter Units.SI.Temperature T0[2*Ns] = fill(290, 2*Ns) "Initial fluid temperature (active if steady_state = false and option_temperature=true)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance and not steady_state and option_temperature));
  parameter Units.SI.SpecificEnthalpy h0[2*Ns] = fill(1e5, 2*Ns) "Initial fluid specific enthalpy (active if steady_state = false and option_temperature=false)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance and not steady_state and not option_temperature));
  ...
  Thermal.HeatTransfer.HeatExchangerWall Wall_3(D = Dc, e = ec, lambda = lambda, cpw = cp, rhow = rho, Ns = Ns*2, L = L3, ntubes = ntubes3, steady_state = steady_state, dynamic_energy_balance = dynamic_energy_balance) annotation(
  ...
  Volumes.VolumeD volumeD(h0 = 790e3, dynamic_energy_balance = dynamic_energy_balance, steady_state = steady_state, diffusion = diffusion, dynamic_mass_balance = dynamic_mass_balance, continuous_flow_reversal = continuous_flow_reversal, ftype = ftype) annotation(
    Placement(transformation(extent = {{-72, -54}, {-54, -36}}, rotation = 0)));
  Volumes.VolumeC volumeC(h0 = 890e3, dynamic_energy_balance = dynamic_energy_balance, dynamic_mass_balance = dynamic_mass_balance, steady_state = steady_state, diffusion = diffusion, continuous_flow_reversal = continuous_flow_reversal, ftype = ftype) annotation(
    Placement(transformation(extent = {{-52, 35}, {-70, 53}}, rotation = 0)));
equation
  ...
  connect(Ce1, volumeD.Ce) annotation(
    Line(points = {{-100, -45}, {-72, -45}}, color = {0, 0, 0}));
  connect(volumeD.Cs1, pipe_3.C1) annotation(
    Line(points = {{-63, -36}, {-64, -36}, {-64, -17}, {-35, -17}}, color = {0, 0, 0}));
  connect(pipe_3.C2, volumeC.Ce1) annotation(
    Line(points = {{60, -17}, {78, -17}, {78, 44}, {-52, 44}}, color = {0, 0, 0}));
  connect(volumeC.Cs, Ce2) annotation(
    Line(points = {{-70, 44}, {-100, 44}}, color = {0, 0, 0}));
  annotation(
