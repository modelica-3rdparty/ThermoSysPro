...
model StaticCondenserHEI "HEI static condenser"
  ...
  parameter Units.SI.Density p_rhoc = 0 "If > 0, fixed fluid density for the hot side";
  parameter Units.SI.Density p_rhof = 0 "If > 0, fixed fluid density for the cold side";
  ...
  parameter Units.SI.Diameter D_tube1 = 25.4e-3 "External diameter of tubes type 1";
  parameter Units.SI.Diameter D_tube2 = 25.4e-3 "External diameter of tubes type 2";
  parameter Units.SI.Diameter D_tube3 = 25.4e-3 "External diameter of tubes type 3";
  parameter Units.SI.Diameter D_tube4 = 25.4e-3 "External diameter of tubes type 4";
  parameter Units.SI.Diameter D_tube5 = 25.4e-3 "External diameter of tubes type 5";
  parameter Units.SI.Diameter D_tube6 = 25.4e-3 "External diameter of tubes type 6";
  ...
  parameter Units.SI.Pressure Poffset = 0 "Offset applied on the pressure provided by HEI";
  parameter Units.SI.MassFlowRate gamma_diff = 1e-4 "Air diffusion conductance (active if diffusion=true in neighbouring volumes)";
  ...
  parameter IF97Region region_c = IF97Region.All_regions "IF97 region for the hot side (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_cs = IF97Region.All_regions "IF97 region at the outlet of the hot side (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_f = IF97Region.All_regions "IF97 region for the cold side (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  ...
  Units.SI.Power W(start = 1e6) "Power exchanged from the hot side to the cold side";
  Units.SI.Temperature Tec(start = 500) "Fluid temperature at the inlet of the hot side";
  Units.SI.Temperature Tsc(start = 400) "Fluid temperature at the outlet of the hot side";
  Units.SI.Temperature Tef(start = 350) "Fluid temperature at the inlet of the cold side";
  Units.SI.Temperature Tsf(start = 350) "Fluid temperature at the outlet of the cold side";
  ...
  Units.SI.CoefficientOfHeatTransfer U1 "Uncorrected heat transfert coefficient";
  ...
  Units.SI.Pressure Pcond(start = 6000) "Expected corrected HEI saturation pressure";
  Units.SI.Temperature Tsat(start = 310) "Expected corrected HEI saturation temperature";
  Units.SI.Power Wcut_off(start = 5e5) "Power exchanged from the hot side to the cold side at Pcut_off";
  Units.SI.Temperature TTD "Terminal Temperature Difference";
  Integer HEI "Applicability of the HEI standard. 0:No - 1:Yes";
  Units.SI.Temperature Tsat_att(start = 310) "Expected HEI saturation temperature";
  Units.SI.SpecificHeatCapacity Cpmf(start = 950) "Average specific heat capacity of the cold water";
  Units.SI.SpecificEnthalpy Hmv(start = 2500000) "Fluid inlets average specific enthalpy";
  Units.SI.SpecificEnthalpy Hml(start = 100000) "Extraction water average specific enthalpy";
  Units.SI.SpecificEnthalpy Hex(start = 100000) "Drain specific enthalpy at the outlet";
  ...
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidInlet Cee "Cooling water inlet" annotation(
    Placement(transformation(extent = {{-112, -72}, {-88, -50}}, rotation = 0)));
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidOutlet Cse "Cooling water outlet" annotation(
    Placement(transformation(extent = {{90, -72}, {114, -50}}, rotation = 0)));
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidOutlet Cex "Extraction water" annotation(
    Placement(transformation(extent = {{-12, -112}, {14, -88}}, rotation = 0), iconTransformation(extent = {{-12, -112}, {14, -88}})));
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidInlet Cvt "Turbine outlet" annotation(
    Placement(transformation(extent = {{-13, 88}, {13, 114}}, rotation = 0), iconTransformation(extent = {{-13, 88}, {13, 114}})));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph profs "Proprietes eau" annotation(
    Placement(transformation(extent = {{50, 80}, {70, 100}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph profe "Proprietes eau" annotation(
    Placement(transformation(extent = {{20, 80}, {40, 100}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph proce "Proprietes eau" annotation(
  ...
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidInlet Cep "Drain inlet" annotation(
    Placement(transformation(extent = {{-112, 8}, {-88, 30}}, rotation = 0)));
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidInlet Cev "Vapor inlet" annotation(
    Placement(transformation(extent = {{-112, 50}, {-88, 72}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph procs "Proprietes eau" annotation(
    Placement(transformation(extent = {{80, -100}, {100, -80}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph promf "Proprietes eau" annotation(
