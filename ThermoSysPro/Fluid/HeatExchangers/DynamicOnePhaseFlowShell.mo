  ...
  if dynamic_energy_balance then
    if steady_state then
      ...
      if option_temperature then
        for i in 2:N loop
          h[i] = Medium.specificEnthalpy_pTX(P[i], T0[i - 1], X);
        end for;
      else
        for i in 2:N loop
          h[i] = h0[i - 1];
...
equation
  ...
  rhoc[1] = Medium.density(Medium.setState_phX(p=P[1], h=h[1], X=X));
  rhoc[N + 1] = Medium.density(Medium.setState_phX(p=P[N + 1], h=h[N + 1], X=X));
