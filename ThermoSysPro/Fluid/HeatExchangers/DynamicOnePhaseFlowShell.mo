...
model DynamicOnePhaseFlowShell "Dynamic one-phase flow shell"
  ...
  parameter Units.SI.Temperature T0[Ns] = fill(290, Ns) "Initial fluid temperature (active if steady_state = false and option_temperature = 1)";
  ...
  Units.SI.Density rhoc[N + 1](start = fill(998, N + 1), nominal = fill(1, N + 1)) "Fluid density at the boundary of node i";
...
protected
  ...
  parameter Units.SI.Area As = B*Ds*dt/dc "Maximum cross sectional area of flow in shell";
  parameter Units.SI.Area A = pi*Ds^2/4 - ntubes*pi*De^2/4 "Liquid cross sectional in shell";
  parameter Units.SI.Diameter Deq = if (option_arrangement == 1) then (3.464*dc^2 - pi*De^2)/(pi*De) else if (option_arrangement == 2) then (4*dc^2 - pi*De^2)/(pi*De) else De "Equivalent diameter of triangle arrangement between tubes";
  parameter Units.SI.Area dSi = pi*De*ntubes*dx1*CSailettes "Internal heat exchange area for a node";
