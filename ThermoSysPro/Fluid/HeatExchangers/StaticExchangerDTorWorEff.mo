...
model StaticExchangerDTorWorEff "Static heat exchanger with fixed delta temperature, delta power or efficiency"
  ...
  parameter IF97Region region_c = IF97Region.All_regions "IF97 region for the hot fluid (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_cs = IF97Region.All_regions "IF97 region of the water at the outlet of the hot side (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_f = IF97Region.All_regions "IF97 region for the cold fluid (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  ...
  Interfaces.Connectors.FluidInlet Ec "Hot inlet" annotation(
    Placement(transformation(extent = {{-50, 31}, {-30, 51}}, rotation = 0)));
  Interfaces.Connectors.FluidInlet Ef "Cold inlet" annotation(
    Placement(transformation(extent = {{-110, -10}, {-90, 10}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet Sf "Cold outlet" annotation(
    Placement(transformation(extent = {{90, -9}, {110, 11}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet Sc "Hot outlet" annotation(
