...
model TestVolumeDTh_FlueGases
  ...
  ThermoSysPro.Fluid.Volumes.VolumeDTh volumeDTh(redeclare package Medium =
        Medium, h(start=2.83057e6))
  ...
  ThermoSysPro.Fluid.BoundaryConditions.SinkQ sinkQ(
    ...
    option_temperature=true) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={40,0})));
  Thermal.BoundaryConditions.HeatSource heatSource(T0={573.15})
...
equation
  ...
  connect(heatSource.C[1], volumeDTh.Cth)
    annotation (Line(points={{0,-28.2},{0,0}}, color={0,0,0}));
