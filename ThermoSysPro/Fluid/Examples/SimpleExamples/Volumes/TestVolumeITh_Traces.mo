...
model TestVolumeITh_Traces
  ...
  ThermoSysPro.Fluid.BoundaryConditions.SourceQ sourceQ(
    redeclare replaceable package Medium = Medium,
    Q0=100,
    SubC0={0.5}) annotation (Placement(transformation(extent={{-56,18},{-36,38}})));
  Thermal.BoundaryConditions.HeatSource heatSource annotation (Placement(transformation(extent={{-12,-28},{8,-48}})));
equation
  connect(volumeITh.Cs1, sink.C) annotation (Line(points={{8,8},{8,30},{20,30}}, color={0,0,0}));
  connect(sourcePQ.C, volumeITh.Ce2) annotation (Line(points={{-30,0},{-12,0}}, color={0,0,0}));
  connect(sourceQ.C, volumeITh.Ce1) annotation (Line(points={{-36,28},{-12,28},{-12,8}}, color={0,0,0}));
  connect(heatSource.C[1], volumeITh.Cth) annotation (Line(points={{-2,-28.2},{-2,0}}, color={0,0,0}));
