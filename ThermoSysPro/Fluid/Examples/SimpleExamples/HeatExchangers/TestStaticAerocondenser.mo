...
model TestStaticAerocondenser
  ...
  SourceH steamSource(
    redeclare package Medium = Medium,
    Q0=100,
    h0=2600000) annotation (Placement(transformation(extent={{-10,50},{10,70}}, rotation=-90,
        origin={4,22})));
  SourceH drainSource(
    redeclare package Medium = Medium,
    Q0=1,
    h0=370000) annotation (Placement(transformation(extent={{-70,-40},{-50,-20}}, rotation=0)));
  SinkDiff condensateSink(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{-10,-70},{10,-50}}, rotation=-90)));
  SourceH airSource(
    redeclare package Medium = Medium_Air,
    fix_h_vol_2=true,
    Q0=1400,
    h0=Medium_Air.specificEnthalpy_pTX(p=100000, T=293.15, X=Medium_Air.X_default)) annotation (Placement(transformation(extent={{60,-10},{40,10}}, rotation=0)));
  SinkPOnly airSink(
    redeclare package Medium = Medium_Air,
    P0=100000) annotation (Placement(transformation(extent={{40,50},{60,70}}, rotation=90)));
equation
  staticAerocondenser.Cws2.h_vol_2 = staticAerocondenser.Cws2.h;
  connect(steamSource.C, staticAerocondenser.Cws1) annotation (Line(points={{0,50},{0,9.16667}}, color={0,0,0}));
  connect(drainSource.C, staticAerocondenser.Cw) annotation (Line(points={{-50,-30},{-9.16667,-7.5}}, color={0,0,0}));
  connect(staticAerocondenser.Cws2, condensateSink.C) annotation (Line(points={{0,-9.16667},{0,-50}}, color={0,0,0}));
  connect(airSource.C, staticAerocondenser.Cair1) annotation (Line(points={{40,0},{9.16667,-9.08333}}, color={0,0,0}));
  connect(staticAerocondenser.Cair2, airSink.C) annotation (Line(points={{9.16667,9.16667},{50,50}}, color={0,0,0}));
