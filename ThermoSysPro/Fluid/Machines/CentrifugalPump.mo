...
model CentrifugalPump "Centrifugal pump"
  ...
  parameter ThermoSysPro.Units.nonSI.AngularVelocity_rpm N = 1400 "Pump angular velocity in rpm (active if input M is not connected)";
  parameter ThermoSysPro.Units.nonSI.AngularVelocity_rpm N_nom = 1400 "Nominal angular velocity in rpm";
  parameter Units.SI.MomentOfInertia J = 10 "Rotating masses moment of inertia (active if dynamic_mech_equation=true)";
  parameter Units.SI.Volume V = 1 "Pump volume (active if dynamic_energy_balance=true)";
  parameter Boolean dynamic_mech_equation = false "true: dynamic mechanical equation - false: static mechanical equation (active if input M is connected)";
  parameter Boolean dynamic_energy_balance = false "true: dynamic energy balance equation - false: static energy balance equation";
  parameter Units.SI.MassFlowRate gamma_diff = 1e-4 "Diffusion conductance (active if diffusion=true in neighbouring volumes)";
  ...
  parameter Integer mode_car = 2 "1:nominal values and coef. c given by parameters - 2:nominal values and coef. c computed from semi-parabolic characteristics";
  parameter Integer mode_car_hn = 2 "1:complete pump head characteristics - 2:semi-parabolic pump head characteristics";
  parameter Integer mode_car_Cr = 2 "1:complete torque characteristics - 2:analytic formula";
  parameter Units.SI.VolumeFlowRate Qv_nom_p = 0.4781 "Nominal volumetric flow (active if mode_car=1)";
  parameter Units.SI.Height hn_nom_p = 22.879 "Nominal pump head (active if mode_car=1)";
  parameter Units.SI.Height rh_nom_p = 0.863 "Nominal pump efficiency (active if mode_car=1)";
  parameter Real F_t[:] = {0.634, 0.643, 0.646, 0.640, 0.629, 0.613, 0.595, 0.575, 0.552, 0.533, 0.516, 0.505, 0.504, 0.510, 0.512, 0.522, 0.539, 0.559, 0.580, 0.601, 0.630, 0.662, 0.692, 0.722, 0.753, 0.782, 0.808, 0.832, 0.857, 0.879, 0.904, 0.930, 0.959, 0.996, 1.027, 1.060, 1.090, 1.124, 1.165, 1.204, 1.238, 1.258, 1.271, 1.282, 1.288, 1.281, 1.260, 1.225, 1.172, 1.107, 1.031, 0.942, 0.842, 0.733, 0.617, 0.500, 0.368, 0.240, 0.125, 0.011, -0.102, -0.168, -0.255, -0.342, -0.423, -0.494, -0.556, -0.620, -0.655, -0.670, -0.670, -0.660, -0.655, -0.640, -0.600, -0.570, -0.520, -0.470, -0.430, -0.360, -0.275, -0.160, -0.040, 0.130, 0.295, 0.430, 0.550, 0.620, 0.634} "Head characteristics (active if mode_car_hn=1)";
  parameter Real G_t[:] = {-0.684, -0.547, -0.414, -0.292, -0.187, -0.105, -0.053, -0.012, 0.042, 0.097, 0.156, 0.227, 0.300, 0.371, 0.444, 0.522, 0.596, 0.672, 0.738, 0.763, 0.797, 0.837, 0.865, 0.883, 0.886, 0.877, 0.859, 0.838, 0.804, 0.758, 0.703, 0.645, 0.583, 0.520, 0.454, 0.408, 0.370, 0.343, 0.331, 0.329, 0.338, 0.354, 0.372, 0.405, 0.450, 0.486, 0.520, 0.552, 0.579, 0.603, 0.616, 0.617, 0.606, 0.582, 0.546, 0.500, 0.432, 0.360, 0.288, 0.214, 0.123, 0.037, -0.053, -0.161, -0.248, -0.314, -0.372, -0.580, -0.740, -0.880, -1.000, -1.120, -1.250, -1.370, -1.490, -1.590, -1.660, -1.690, -1.770, -1.650, -1.590, -1.520, -1.420, -1.320, -1.230, -1.100, -0.980, -0.820, -0.684} "Torque characteristics (active if mode_car_Cr=1)";
  parameter Real c_p = 1.288 "Dimensionless coef. of the semi-parabolic pump head characteristics (active if mode_car=1 and mode_car_hn=2)";
  constant Real b = 2 "Dimensionless coef. of the parabolic pump efficiency characteristics";
  parameter Real hn_coef[2] = {-88.67, 43.15} "Coef. of the semi-parabolic pump head characteristics (active if mode_car=2)";
  parameter Real rh_coef[2] = {-3.7751, 3.61} "Coef. of the parabolic pump efficiency characteristics (active if mode_car=2)";
  ...
  ElectroMechanics.Connectors.MechanichalTorque M annotation(
...
/* Energy balance equation */
  if dynamic_energy_balance then
    ...
    deltaH = ThermoSysPro.Functions.SmoothCond(Q, g*hn/ThermoSysPro.Functions.SmoothMax(rh, rh_min), g*hn/ThermoSysPro.Functions.SmoothMin(rh, -rh_min));
  end if;
/* Nominal quantities */
  w_nom = pi/30*N_nom;
  if (mode_car == 1) then
...
/* Angle theta */
  theta = noEvent(if (abs(w_a) > w_a_min) then atan(Qv_a/w_a) else if (abs(Qv_a) < Qv_a_min) then (if (w_a > 0) then 0 else -pi) else if (Qv_a > 0) then pi/2 else -pi/2);
...
/* Pump efficiency */
  rh_a = if noEvent(abs(w_a) > w_a_min) then -(b - 1)*(Qv_a/w_a)^2 + b*Qv_a/w_a else -(b - 1)*(Qv_a/w_a_min)^2 + b*Qv_a/w_a_min;
/* Motor power */
  Wm = Cm*w;
...
/* Fluid thermodynamic properties */
  ...
  annotation(
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 0}, fillPattern = FillPattern.Solid, fillColor = {127, 255, 0}), Polygon(points = {{-30, 40}, {60, 0}, {-30, -40}, {-30, 40}}, lineColor = {0, 0, 255}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid)}),
    Window(x = 0.03, y = 0.02, width = 0.95, height = 0.95),
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillPattern = FillPattern.Solid, fillColor = DynamicSelect({127, 255, 0}, fill_color_singular)), Polygon(points = {{-30, 40}, {60, 0}, {-30, -40}, {-30, 40}}, lineColor = {0, 0, 255}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid)}),
    Documentation(info = "
