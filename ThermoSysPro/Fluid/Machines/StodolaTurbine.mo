...
model StodolaTurbine "Multistage turbine group using Stodola's ellipse"
  ...
  parameter IF97Region region_e = IF97Region.All_regions "IF97 region before expansion (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_s = IF97Region.All_regions "IF97 region after expansion (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  parameter IF97Region region_ps = IF97Region.All_regions "IF97 region after isentropic expansion (active for IF97 water/steam only)" annotation(
    Evaluate = true,
    Dialog(enable = (ftype == FluidType.WaterSteam), tab = "Fluid", group = "Fluid properties"));
  ...
  Interfaces.Connectors.FluidInlet Ce annotation(
    Placement(transformation(extent = {{-111, -10}, {-91, 10}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet Cs annotation(
...
/* Fluid thermodynamic properties after the isentropic expansion */
  ...
  annotation(
    ...
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Polygon(points = {{-100, 40}, {-100, -40}, {100, -100}, {100, 100}, {-100, 40}}, lineColor = {0, 0, 0}, fillColor = DynamicSelect({127, 255, 0}, fill_color_singular), fillPattern = FillPattern.Solid), Line(points = {{0, -70}, {0, -90}}, color = {0, 0, 0}, thickness = 0.5)}),
