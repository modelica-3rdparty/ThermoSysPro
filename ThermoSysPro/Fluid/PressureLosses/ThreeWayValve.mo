...
model ThreeWayValve "Three way valve"
  ...
  parameter Boolean diffusion = true "true: energy balance equation with diffusion - false: energy balance equation without diffusion";
  ...
  ThermoSysPro.Fluid.PressureLosses.ControlValve Valve1(Cvmax = Cvmax1, caract = caract1, mode_caract = mode_caract1, p_rho = p_rho, region = region, gamma_diff = gamma_diff) annotation(
    Placement(transformation(origin = {-6, -50}, extent = {{10, -10}, {-10, 10}}, rotation = 90)));
  Interfaces.Connectors.FluidOutlet C3 annotation(
    Placement(transformation(extent = {{-10, -110}, {10, -90}}, rotation = 0)));
  ThermoSysPro.Fluid.PressureLosses.ControlValve Valve2(Cvmax = Cvmax2, caract = caract2, mode_caract = mode_caract2, p_rho = p_rho, region = region, gamma_diff = gamma_diff) annotation(
  ...
  Volumes.VolumeA VolumeA1(V = V, p_rho = p_rho, dynamic_energy_balance = dynamic_energy_balance, diffusion = diffusion, continuous_flow_reversal = continuous_flow_reversal, ftype = ftype, region = region, dynamic_composition_balance = dynamic_composition_balance, Xco20 = Xco20, Xh2o0 = Xh2o0, Xo20 = Xo20, Xso20 = Xso20) annotation(
    Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ThermoSysPro.Fluid.PressureLosses.PipePressureLoss PerteDP1(K = 0, region = region, gamma_diff = gamma_diff) annotation(
...
equation
  ...
  connect(Add1.y, Valve1.Ouv) annotation(
    Line(points = {{-19, 50}, {-10, 50}, {-10, 26}, {-28, 26}, {-28, -50}, {-17, -50}}, color = {0, 0, 255}));
  connect(VolumeA1.Cs1, Valve2.C1) annotation(
    Line(points = {{10, 0}, {40, 0}}, color = {0, 0, 255}));
  connect(VolumeA1.Cs2, Valve1.C1) annotation(
    Line(points = {{0, -10}, {0, -40}, {8.88178e-016, -40}}, color = {0, 0, 255}));
  connect(Valve1.C2, C3) annotation(
    Line(points = {{-8.88178e-016, -60}, {-8.88178e-016, -78}, {0, -78}, {0, -100}}, color = {0, 0, 255}));
