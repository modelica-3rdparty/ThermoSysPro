...
model DynamicReliefValve "Dynamic relief valve"
  ...
  parameter Integer mode_caract = 0 "0:linear characteristics - 1:characteristics is given by caract[] - 2:characteristics for conic clapper" annotation(
    Evaluate = true);
  parameter Integer option_interpolation = 1 "1: linear interpolation - 2: spline interpolation (active if mode_caract=1)" annotation(
    Evaluate = true,
    Dialog(enable = (mode_caract == 1)));
  parameter Boolean mech_steady_state = true "true: start from mechanical steady state - false: start from 0" annotation(
    Evaluate = true);
  parameter Units.SI.Length z0 = 0 "Initial clapper elevation, between 0 and z_max. 0:valve closed - z_max: valve fully open (active if mech_steady_state=false)" annotation(
    Evaluate = true,
    Dialog(enable = not mech_steady_state));
