...
model VolumeATh "Mixing volume with 2 inlets, 2 outlets and 1 thermal input"
  ...
  replaceable package Medium_CoolProp =
      Modelica.Media.Interfaces.PartialTwoPhaseMedium               "CoolProp Medium" annotation(Evaluate=true, Dialog(tab="Fluid", group="CoolProp properties (enable if FluidType.CoolPropMedium)",enable=(ftype == FluidType.CoolPropMedium)));
...
equation
  ...
  if (p_rho > 0) then
    ...
    if fluid==8 then
//       rho = Medium_CoolProp.density(Medium_CoolProp.setState_ph(p=P, h=h, phase=0));
