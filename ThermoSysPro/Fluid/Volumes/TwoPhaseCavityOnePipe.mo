...
model TwoPhaseCavityOnePipe "TwoPhaseCavity for one shell pass "
  ...
  parameter Real Vf0 = 0.5 "Fraction of initial water volume in the drum (active if dynamic_energy_balance=true and steady_state=false)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance and not steady_state));
  parameter Units.SI.AbsolutePressure P0 = 1.e5 "Fluid initial pressure (active if dynamic_energy_balance=true and steady_state=false)" annotation(
    Evaluate = true,
    Dialog(enable = dynamic_energy_balance and not steady_state));
  ...
  Units.SI.SpecificEnthalpy hvIn(start = 2400000) "Steam average specific enthalpy input cavity";
  ...
  Units.SI.MassFlowRate QcondS(start = 100) "Splitter mass flow rate of the liquid phase from the two-phase input";
  ...
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph prol "Propri鴩s de l'eau dans le ballon" annotation(
    Placement(transformation(extent = {{-250, 70}, {-210, 110}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph prov "Propri鴩s de la vapeur dans le ballon" annotation(
  ...
  Interfaces.Connectors.FluidInlet CvBP "Steam input" annotation(
    Placement(transformation(extent = {{-86, 50}, {-66, 70}}, rotation = 0)));
  Interfaces.Connectors.FluidOutlet Cl "Water output" annotation(
  ...
  Interfaces.Connectors.FluidInlet Ce "Water input" annotation(
    Placement(transformation(extent = {{-219, 19}, {-199, 39}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph proe "Propri鴩s de l'eau dans le ballon" annotation(
    Placement(transformation(extent = {{-250, -106}, {-210, -66}}, rotation = 0)));
  Properties.WaterSteam.Common.ThermoProperties_ph provIn "Propri鴩s de la vapeur dans le ballon" annotation(
    Placement(transformation(extent = {{0, 70}, {40, 110}}, rotation = 0)));
  Interfaces.Connectors.FluidInlet CvGCT "Steam input" annotation(
...
/* Check that incoming fluids are compatible with fluid in volume */
  ...
  fluids[4] = CvBP.ftype;
  fluids[5] = Cl.ftype;
  assert(ThermoSysPro.Fluid.Interfaces.PropertyInterfaces.isCompatible(fluids), "TwoPhaseCavityOnePipe: fluids mixing in volume are not compatible with each other");
