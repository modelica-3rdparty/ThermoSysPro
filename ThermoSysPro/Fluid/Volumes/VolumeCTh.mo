...
model VolumeCTh "Mixing volume with 3 inlets, 1 outlet and 1 thermal input"
  ...
  ThermoSysPro.Fluid.Interfaces.Connectors.FluidInlet Ce2 annotation(
    Placement(transformation(extent = {{-10, 90}, {10, 110}}, rotation = 0), iconTransformation(extent = {{-10, 90}, {10, 110}})));
...
/* Check that incoming fluids are compatible with fluid in volume */
  ...
  fluids[4] = Ce3.ftype;
  fluids[5] = Cs.ftype;
  assert(ThermoSysPro.Fluid.Interfaces.PropertyInterfaces.isCompatible(fluids), "VolumeCTh: fluids mixing in volume are not compatible with each other");
