...
model DynamicOnePhaseFlowShell "Dynamic one-phase flow shell"
  ...
  Units.SI.ThermalConductivity k[N - 1](start = fill(0.6, N - 1), nominal = fill(0.6, N - 1)) "Fluid thermal conductivity in node i";
  ...
  Units.SI.SpecificHeatCapacity cp[N - 1](start = fill(4000, N - 1), nominal = fill(4000, N - 1)) "Fluid specific heat capacity";
  ...
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat lsat[N - 1] annotation(
    Placement(transformation(extent = {{-20, 80}, {0, 100}}, rotation = 0)));
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat vsat[N - 1] annotation(
...
protected
  ...
  parameter Units.SI.Area dSi = pi*De*ntubes*dx1 "Internal heat exchange area for a node";
