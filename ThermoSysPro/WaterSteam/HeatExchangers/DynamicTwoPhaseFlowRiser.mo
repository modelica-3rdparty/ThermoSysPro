...
model DynamicTwoPhaseFlowRiser "Riser: Dynamic two-phase flow pipe"
  ...
  parameter Units.SI.Temperature T0[Ns] = fill(300, Ns) "Initial fluid temperature (active if steady_state = false and option_temperature = 1)" annotation(
    Evaluate = false);
  parameter Units.SI.SpecificEnthalpy h0[Ns] = fill(1e5, Ns) "Initial fluid specific enthalpy (active if steady_state = false and option_temperature = 2)";
  ...
  parameter Boolean simplified_dynamic_energy_balance = true "true: simplified dynamic energy balance equation - false: full dynamic energy balance equation (active if dynamic_mass_balance=true)";
  ...
  Units.SI.ThermalConductivity kl[N - 1](start = fill(0.6, N - 1), nominal = fill(0.6, N - 1)) "Thermal conductivity in node i for the liquid";
  Units.SI.ThermalConductivity kv[N - 1](start = fill(0.03, N - 1), nominal = fill(0.03, N - 1)) "Thermal conductivity in node i for the vapor";
  ...
  Units.SI.SpecificHeatCapacity cpl[N - 1](start = fill(4000, N - 1), nominal = fill(4000, N - 1)) "Specific heat capacity for the liquid";
  Units.SI.SpecificHeatCapacity cpv[N - 1](start = fill(2000, N - 1), nominal = fill(2000, N - 1)) "Specific heat capacity for the vapor";
...
//hi_2[i] =  max(1970*exp(P[i + 1]/43.5e5)*(Tp2[i] - Tsat[i]), 5000);
    ...
    hcl[i] = noEvent(if ((Rel1[i] > 1.e-6) and (Prl[i] > 1.e-6)) then 0.023*kl[i]/D*Rel1[i]^0.8*Prl[i]^0.4 else 0);
    hcv[i] = noEvent(if ((Rev1[i] > 1.e-6) and (Prv[i] > 1.e-6)) then 0.023*kv[i]/D*Rev1[i]^0.8*Prv[i]^0.4 else 0);
...
/* Fluid densities at the boundaries of the nodes */
  ...
  annotation(
    ...
    Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, grid = {2, 2}), graphics = {Rectangle(extent = {{-100, 0}, {100, -20}}, lineColor = {28, 108, 200}, fillColor = {170, 213, 255}, fillPattern = FillPattern.Solid), Rectangle(extent = {{-100, -20}, {100, -40}}, lineColor = {28, 108, 200}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid), Line(points = {{-60, 0}, {-60, -40}}), Line(points = {{-20, 0}, {-20, -40}}), Line(points = {{20, 0}, {20, -40}}), Line(points = {{60, 0}, {60, -40}}), Rectangle(extent = {{-100, 60}, {100, 40}}, lineColor = {28, 108, 200}, fillColor = {170, 213, 255}, fillPattern = FillPattern.Solid), Rectangle(extent = {{-100, 40}, {100, 20}}, lineColor = {28, 108, 200}, fillColor = {85, 170, 255}, fillPattern = FillPattern.Solid), Rectangle(extent = {{-100, 20}, {100, 0}}, lineColor = {28, 108, 200}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid), Line(points = {{-60, 60}, {-60, 20}}), Line(points = {{-20, 60}, {-20, 20}}), Line(points = {{20, 60}, {20, 20}}), Line(points = {{60, 60}, {60, 20}})}),
