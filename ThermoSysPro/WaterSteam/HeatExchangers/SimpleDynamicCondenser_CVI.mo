...
model SimpleDynamicCondenser_CVI
  parameter ThermoSysPro.Units.SI.Volume V=1 "Cavity volume";
  parameter ThermoSysPro.Units.SI.Area A=1 "Cavity cross-sectional area";
  parameter Real Vf0=0.5
    "Fraction of initial water volume in the drum (active if steady_state=false)";
  parameter ThermoSysPro.Units.SI.AbsolutePressure P0=10000
  ...
  parameter ThermoSysPro.Units.SI.Area Avl=1
  ...
  parameter ThermoSysPro.Units.SI.Diameter D=0.02 "Pipe internal diameter";
  parameter ThermoSysPro.Units.SI.Length e=2.e-3 "Wall thickness";
  ...
  parameter ThermoSysPro.Units.SI.Length rugosrel=0.0007 "Pipe roughness";
  ...
  parameter ThermoSysPro.Units.SI.Area At=ntubes*pi*D^2/4
...
public
  ThermoSysPro.Units.SI.Density rhom(start=998) "Liquid phase density";
  ...
  ThermoSysPro.Units.SI.SpecificEnthalpy hl "Liquid phase spepcific enthalpy";
  ThermoSysPro.Units.SI.SpecificEnthalpy hv "Gas phase spepcific enthalpy";
  ...
  ThermoSysPro.Units.SI.Power Wout
  ...
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat vsat
          ...
          extent={{12,38},{32,58}}, rotation=0)));
  ThermoSysPro.WaterSteam.Connectors.FluidInlet Cv(redeclare package Species =
  ...
  ThermoSysPro.WaterSteam.Connectors.FluidOutlet Cl(redeclare package Species
      = Species) annotation (Placement(transformation(extent={{-8,-110},{12,-90}},
          rotation=0)));
...
public
  ThermoSysPro.WaterSteam.Connectors.FluidInlet Cee(redeclare package Species
      = Species) annotation (Placement(transformation(extent={{-110,-36},{-90,-16}},
          rotation=0)));
  ThermoSysPro.WaterSteam.Connectors.FluidOutlet Cse(redeclare package Species
      = Species) annotation (Placement(transformation(extent={{90,-30},{110,-10}},
          rotation=0)));
  ThermoSysPro.Properties.WaterSteam.Common.ThermoProperties_ph proe
    ...
    //ATTENTION^^^^: VERIFY WHICH TEMPERATURE TO PASS TO SUBMASSBALANCE (T=T1 FOR NOW...)

  ThermoSysPro.WaterSteam.Connectors.FluidOutlet CVI(redeclare package Species
      = Species)
  ...
  ThermoSysPro.ConvectedQuantities.Components.MassBalance_HeterogeneousPhases sub_massBalance(
    ...
    rho=rhov,
    x=xv,
    T=Tv) annotation (Placement(transformation(extent={{-92,38},{-44,86}})));
initial equation
...
equation
  ...
  sub_massBalance.mix_out_liq.SubC = {Cl.SubC};
  sub_massBalance.mix_out_gas.SubC = {CVI.SubC};
 ...
 if (cardinality(CVI) == 0) then
  ...
  BHv = Cv.Q*(Cv.h - (hv - P/rhov)) + Qevap*(vsat.h - (hv - P/rhov)) - Qcond*(lsat.h - (hv - P/rhov)) - CVI.Q*(CVI.h - (hv - P/rhov)) - Wvl + Wout;
  ...
  /* Thermal power exchanged from the steam to the pipes */
  Wout = -(Cv.Q*Cv.h - (Cl.Q*Cl.h + CVI.Q*CVI.h));
...
end SimpleDynamicCondenser_CVI;
