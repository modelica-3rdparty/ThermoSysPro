...
model VolumeATh_derH
  "Mixing volume with 2 inlets and 2 outlets and thermal input"
...
public
  ...
  ConvectedQuantities.Components.MassBalance              sub_massBalance(
    ...
    rho=rho,
    T=T)
    annotation (Placement(transformation(extent={{-106,36},{-66,76}})));
initial equation
...
equation
  ...
  if (cardinality(Ce2) == 0) then
    ...
    Ce2.h = 1.e5;
    Ce2.b = true;
    Ce2.SubC = fill(0,size(Ce1.SubC,1));
  end if;
