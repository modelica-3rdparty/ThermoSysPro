...
model VolumeA_C0 "Mixing volume with 2 inlets and 2 outlets"
 ...
 WaterSteam.Connectors.FluidInlet Ce1(redeclare package Species = Species)
  ...
  WaterSteam.Connectors.FluidOutlet Cs2(redeclare package Species = Species)
    annotation (Placement(transformation(extent={{-10,-110},{10,-90}}, rotation=
           0)));
  ThermoSysPro.ConvectedQuantities.Components.MassBalance_C0 sub_massBalance(
...
//   else
//     if dynamic_mass_balance then
//       //P = P0; //ATTENTION: Supression du steady state pour p et h
//       der(P) = 0;
//     end if;
//
//     //h = h0; //ATTENTION: Supression du steady state pour p et h
//     der(h) = 0;
//   end if;
...
equation
  ...
  0 = Ce1.Q + Ce2.Q - Cs1.Q - Cs2.Q;
//   if dynamic_mass_balance then
//     V*(pro.ddph*der(P) + pro.ddhp*der(h)) = BQ; //ATTENTION: Supression du dynamic mass balance pour p et h
//   else
//     0 = BQ;
//   end if;
  ...
  V*rho*der(h) = Ce1.Q*Ce1.h + Ce2.Q*Ce2.h - Cs1.Q*Cs1.h - Cs2.Q*Cs2.h;

//   if dynamic_mass_balance then
//     V*((h*pro.ddph - 1)*der(P) + (h*pro.ddhp + rho)*der(h)) = BH; //ATTENTION: Supression du dynamic mass balance pour p et h
//   else
//     V*rho*der(h) = BH;
//   end if;
