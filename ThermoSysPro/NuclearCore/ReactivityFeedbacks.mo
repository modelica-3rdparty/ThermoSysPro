...
model ReactivityFeedbacks "This module calculates the neutronic feedback reactions due to the control rods, the Doppler effect, the moderator, 
  ...
  parameter ThermoSysPro.Units.SI.Temperature t0_doppler = 944 "Doppler Temperature associated (K)";
  ...
  parameter ThermoSysPro.Units.SI.Temperature Tref_fuel(start = 973.15, fixed = false) "fuel effective reference temperature (K)";
  parameter Real XPosgYEdiffg[27, 2] = [25*8, -62.5; 24*8, -62.5; 23*8, -62.5; 22*8, -62.5; 21*8, -62.5; 20*8, -62.5; 19*8, -62.5; 18*8, -62.5; 17*8, -62.5; 16*8, -62.5; 15*8, -62.5; 14*8, -62.5; 13*8, -62.5; 12*8, -62.5; 11*8, -62.5; 10*8, -62.5; 9*8, -62.5; 8*8, -62.5; 7*8, -62.5; 6*8, -62.5; 5*8, -62.5; 4*8, -62.5; 3*8, -62.5; 2*8, -62.5; 8, -62.5; 0, -62.5; 0, 0] "XPosgYEdiffg (input = first column, output = second column)";
  parameter Real XPosrYEdiffr[27, 2] = [25*8, -3; 24*8, -3; 23*8, -3; 22*8, -3; 21*8, -3; 20*8, -3; 19*8, -3; 18*8, -3; 17*8, -3; 16*8, -3; 15*8, -3; 14*8, -3; 13*8, -3; 12*8, -3; 11*8, -3; 10*8, -3; 9*8, -3; 8*8, -3; 7*8, -3; 6*8, -3; 5*8, -3; 4*8, -3; 3*8, -3; 2*8, -3; 8, -3; 0, -3; 0, 0] "XPosrYEdiffr (input = first column, output = second column)";
  ...
  Real ReacBG "Reactivity given by the control bars of group G (pcm)";
  Real ReacBR "Reactivity given by the control bars of group R (pcm)";
  ...
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal SortieReac annotation(
    extent = [100, 70; 120, 90],
    Placement(transformation(extent = {{100, -16}, {120, 4}}, rotation = 0), iconTransformation(extent = {{100, -16}, {120, 4}})));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreeCbore annotation(
    extent = [-120, -70; -100, -50],
    Placement(transformation(extent = {{-120, -70}, {-100, -50}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreeT_CoreAv annotation(
    extent = [-120, -30; -100, -10],
    Placement(transformation(extent = {{-120, -30}, {-100, -10}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreeT_fuel annotation(
    extent = [-120, 10; -100, 30],
    Placement(transformation(extent = {{-120, 10}, {-100, 30}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreePosG annotation(
    extent = [-120, 50; -100, 70],
    Placement(transformation(extent = {{-120, 50}, {-100, 70}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreeVelR annotation(
    extent = [-120, 90; -100, 110],
    Placement(transformation(extent = {{-120, 90}, {-100, 110}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreeCxenon annotation(
    extent = [-120, -110; -100, -90],
    Placement(transformation(extent = {{-120, -110}, {-100, -90}}, rotation = 0)));
protected
  parameter Real XPosg[1, :] = transpose(matrix(XPosgYEdiffg[:, 1])) "Inputs of the table";
  parameter Real YEdiffg[1, :] = transpose(matrix(XPosgYEdiffg[:, 2])) "Outputs of the table";
  parameter Integer ng[1, 1] = [size(XPosg, 2)] "Size of the table";
  parameter Real XPosr[1, :] = transpose(matrix(XPosrYEdiffr[:, 1])) "Inputs of the table";
  parameter Real YEdiffr[1, :] = transpose(matrix(XPosrYEdiffr[:, 2])) "Outputs of the table";
