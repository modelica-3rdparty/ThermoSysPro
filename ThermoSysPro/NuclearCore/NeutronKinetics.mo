...
model NeutronKinetics "This module contains a model of the neutronic flux with six groups of
  ...
  parameter ThermoSysPro.Units.SI.Time Tlife = 23.27e-6 "Average lifetime of the prompt neutrons in the core (s)";
  parameter Real Kuo2 = 1 "Ratio between the power produced in the fuel and the total power";
  parameter Real Lambda[6] = {0.0125, 0.0308, 0.1143, 0.3103, 1.2331, 3.289} "Radioactivity constants of the groups of the delayed neutrons (1/s)";
  parameter Real Beta[6] = {0.00021, 0.00142, 0.00131, 0.00274, 0.000932, 0.000313} "Fraction of delayed neutrons in each group with respect to the total number of neutrons emitted per fission";
  ...
  ThermoSysPro.Units.SI.Power Pndelay[6](start = {2570e9, 7052.7e9, 1753.3e9, 1350.8e9, 115.62e9, 14.558e9}) "Power given by the 6 groups of delayed neutrons (W)";
  ...
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreeReac annotation(
    extent = [-120, 30; -100, 50],
    Placement(transformation(extent = {{-120, 30}, {-100, 50}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal EntreePres annotation(
    extent = [-120, -50; -100, -30],
    Placement(transformation(extent = {{-120, -50}, {-100, -30}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal SortiePneut annotation(
    extent = [100, 40; 120, 60],
    Placement(transformation(extent = {{100, 40}, {120, 60}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal SortiePtot annotation(
    extent = [100, 6; 120, 26],
    Placement(transformation(extent = {{100, 6}, {120, 26}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal SortiePuo2 annotation(
    extent = [100, -26; 120, -6],
    Placement(transformation(extent = {{100, -26}, {120, -6}}, rotation = 0)));
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal SortiePh2o annotation(
    extent = [100, -60; 120, -40],
    Placement(transformation(extent = {{100, -60}, {120, -40}}, rotation = 0)));
protected
